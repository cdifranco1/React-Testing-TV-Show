...
import App from './App';
import { render, waitFor, getByText, getByPlaceholderText, fireEvent } from '@testing-library/react';
...
test('data fetched from api is rendered on load', async () => {
  ...
  await waitFor(() => expect(queryAllByText(/stranger things/i)).toBeTruthy())

  expect(mockFetchShow).toHaveBeenCalledTimes(1)

  const dropdown = getByText(/select a season/i)
  
  userEvent.selectOptions(dropdown, ["Season 1"])
